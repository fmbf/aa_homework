...
def below_average_years
  ...
  # Movie.select(:yr, 'COUNT(*)').where('score < 5').group(:yr).order('yr DESC')
...
def pulp_fiction_actors
  ...
  Actor.select(:id, :name).joins(:movies).where("title = 'Pulp Fiction'")#('title = "Pulp Fiction"') #joins(:castings)
end
...
def uma_movies
  ...
  # Movie.select(:id, :title, :yr).joins(:actors).where
